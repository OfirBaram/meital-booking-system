...
- Name: admin-render.spec.js >> renderDiarySlots — no inline onclick in DOM >> clicking a toggle-diary-slot button calls the API without JS error
- Location: tests\e2e\admin-render.spec.js:185:3
...
Locator: locator('#js-diary-slots [data-action="toggle-diary-slot"]:not([disabled])').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#js-diary-slots [data-action="toggle-diary-slot"]:not([disabled])').first()
